...
import { fetchFAQFile } from "~/redux/helpers/user";
...
class FAQViewer extends Component {
  ...
  fetchFile = () => {
    ...
    fetchFAQFile(routeParam).then((response) => {
      if (!response) {
        this.setState({
          isLoading: false,
          dataType: 0,
          data: response.data,
          dataUrl: t("pdfViewer.label.noFaq"),
        });
        return false;
      }
        ...
        <DialogTitle id="customized-dialog-title" style={{paddingTop: 0, paddingBottom: 0}}>
          <Box display="flex" width="100%" position="relative">
            <Box p={1} pl={3} display="flex" alignItems="center">
              <Typography variant="h3" style={{fontWeight: 'bold'}}>{t("footer.label.FAQs")}</Typography>
            </Box>
            <Box p={1} flexGrow={1}>
              {dataType === 1 ? (
                <IconButton
