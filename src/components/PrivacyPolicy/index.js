...
import { fetchPrivacyPolicyFile } from "~/redux/helpers/user";
import { withTranslation } from "react-i18next";
import { compose } from "redux";

import { styles } from "./styles";
class PrivacyPolicyViewer extends Component {
  state = {
    numPages: null,
    pageNumber: 1,
    data: null,
  ...
  fetchFile = () => {
    ...
    fetchPrivacyPolicyFile(routeParam).then((response) => {
      if (!response) {
        this.setState({
          isLoading: false,
          dataType: 0,
          data: response.data,
          dataUrl: t("pdfViewer.label.noPrivacyPolicy"),
        });
        return false;
      }
  ...
  render() {
    ...
    const { dataType, dataUrl, isLoading } =
      this.state;
          ...
          <Box display="flex" width="100%" position="relative">
            <Box p={1} pl={3} display="flex" alignItems="center">
              <Typography variant="h3" style={{fontWeight: 'bold'}}>
                {t("footer.labelLink.PrivacyPolicy")}{" "}
              </Typography>
            </Box>
            <Box p={1} flexGrow={1}>
              {dataType == 1 ? (
          ...
          {open ? (
            ...
            >
              ...
              {dataType === 1 ? (
                <iframe
                  id="privacyPolicy"
                  title={t("pdfViewer.label.privacyPolicy")}
                  name={t("pdfViewer.label.privacyPolicy")}
                  src={dataUrl}
