...
import PFB from '~/assets/icons/Prepaidcard.svg';
import DDC from '~/assets/icons/paymentMethods/DDC_dashboard.svg';
import Zelle from '~/assets/icons/paymentMethods/USBankZelle_main.svg';
...
function USBankPaymentTable({
  ...
  const { details, editType, type, loadData, methodName, user, accounts } =
    props;
  ...
  const renderpaymentDetails = (type, priority, PaymentTypeDesc) => {
    return (
      <Box display='flex' alignItems='center'>
        ...
        <Box px={2} display='flex' alignItems='center'>
          <img src={getPaymentTypeIcon(type)} alt={'Payments'} width={type==="ZEL" && 18}/>
        </Box>
        <Hidden smDown>
          <span className={customClasses.flagLabel}>{PaymentTypeDesc}</span>
        </Hidden>
      </Box>
    );
  };
...
//export default withTranslation('common')(USBankPaymentTable);
export default connect((state) => ({
  ...state.accounts,
}))(withTranslation('common')(USBankPaymentTable));
