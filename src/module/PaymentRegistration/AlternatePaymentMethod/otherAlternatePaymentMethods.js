...
import ACH_Consumer from "~/assets/icons/paymentMethods/ACH_Consumer.svg";
...
import CHK_Consumer from "~/assets/icons/paymentMethods/Check_alternate.svg";
...
class OtherAlternatePaymentMethods extends Component {
  componentDidMount = () => {
    this.props.dispatch(fetchConsumerPaymentTypesList()).then((response) => {
      if (response) {
  ...
  updatePaymentList = () => {
    ...
    if (alternatePaymentMethods.length === 1) {
      this.props.dispatch(
  ...
  render() {
    ...
    const isSelectable =
      accounts.secondaryPaymentMethodList?.alternatePaymentMethods?.length === 1
        ? false
        : true;
    return (
      <Grid container spacing={3} alignItems="stretch">
        {accounts.secondaryPaymentMethodList?.data?.alternatePaymentMethods ? (
          accounts.secondaryPaymentMethodList.data.alternatePaymentMethods.map(
            (paymentMethod, index) => {
              return this.renderPaymentBox(paymentMethod, index, isSelectable);
            }
          )
        ) : accounts.secondaryPaymentMethodList?.error ? (
          <Notification
            variant={"error"}
            message={accounts.secondaryPaymentMethodList.error}
            handleClose={() => {}}
          />
        ) : (
          <CircularProgress />
