...
const InstantBanking = (props) => {
      ...
      .then((res) => {
        if (!res.error) {
          ...
          if (connectURL) {
            ...
            setShowLoader(false);
            handleNotification(t('paymentRegistration.somethingWentWrong'), 'error');
          }
        } else {
          setShowLoader(false);
          handleNotification(t('paymentRegistration.invalidReq'), 'error');
        }
      })
      .catch((err) => {
        setShowLoader(false);
  ...
  const handleSaveData = () => {
    if (!selectedAccount) {
      handleNotification(t('paymentRegistration.selectAnAccount'), 'error');
    } else {
      getAccountData();
    }
  };
          ...
          >
            {showAccountList ? (
              <>
                ...
                >{`${t('paymentRegistration.shareAccount')} ${brandInfo.clientName}`}</Typography>
              </>
            ) : (
              <></>
            )}
