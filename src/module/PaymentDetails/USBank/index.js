...
const USBankPaymentDetails = ({
  ...
  const renderpaymentDetails = (type, priority) => {
    console.log('renderpaymentDetails', paymentDetail.PaymentType);
    switch (type) {
      case 'ACH':
      case 'RTP':
        return (
      ...
      case 'DDC':
        return (
          <span className={customClasses.flagcontainer}>
            ...
            <span>{starredMaskCard(paymentDetail.CardNumber)}</span>
          </span>
        );
...
export default connect((state) => ({ ...state.accounts,...state.user }))(
  compose(withTranslation('common'))(USBankPaymentDetails)
);
